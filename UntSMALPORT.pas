...
type
  ...
  private
   ...
   procedure StarSSSmallportService;
...
procedure Register;
begin
  RegisterComponents('Kamran Component', [TKMSmallPort]);
end;
...
constructor TKMSmallPort.Create(aOwner: TComponent);
begin
...
destructor TKMSmallPort.Destroy;
begin
...
function TKMSmallPort.OpenDriver :boolean;
...
procedure TKMSmallPort.CloseDriver;
begin
 if hDevice <> INVALID_HANDLE_VALUE then
 begin
  CloseHandle(hDevice);
...
function TKMSmallPort.IsOpen: boolean;
begin
 if hDevice = INVALID_HANDLE_VALUE then Result := FALSE
 else Result := TRUE;
end;

procedure TKMSmallPort.ReOpen(value: boolean);
begin
  if IsOpen then CloseDriver
  else OpenDriver;
end;

function TKMSmallPort.ReadByte(Index: WORD): WORD;
var
 value: word;
begin
 if hDevice = INVALID_HANDLE_VALUE then
...
procedure TKMSmallPort.WriteByte(Index: WORD; value: WORD);
begin
  if hDevice = INVALID_HANDLE_VALUE then
  begin
    PortResult:=false;
...
function TKMSmallPort.ReadWord(port: WORD): WORD;
var
 value: word;
begin
 if hDevice = INVALID_HANDLE_VALUE then
...
procedure TKMSmallPort.WriteWord(port: WORD; value: WORD);
begin
  if hDevice = INVALID_HANDLE_VALUE then
  begin
    PortResult:=false;
...
function TKMSmallPort.ReadDWord(port: WORD): DWORD;
var
 value: DWORD;
begin
 if hDevice = INVALID_HANDLE_VALUE then
...
procedure TKMSmallPort.WriteDWord(port: WORD; value: DWORD);
begin
  if hDevice = INVALID_HANDLE_VALUE then
  begin
    PortResult:=false;
...
procedure TKMSmallPort.SpeakerOn(wTone: WORD);
begin
...
procedure TKMSmallPort.SpeakerOff;
begin
  Port[$61]:=(Port[$61]AND $FC);
end;

procedure TKMSmallPort.Sound(wFrq: WORD; delay: DWORD);
var Tone: WORD;
begin
...
function TKMSmallPort.GetWindowsVersion: DWORD;
var
  ver: TOSVersionInfoA;
begin
...
procedure TKMSmallPort.OpenServiceManager;
begin
  if hManager = 0 then
  begin
    dwLastError := 0;
...
procedure TKMSmallPort.CloseServiceManager;
begin
  if(hManager <> 0)then
  begin
...
procedure TKMSmallPort.InstallSmallPortDriver;
var
  hService: SC_HANDLE;
  DriverPath: string;
...
procedure TKMSmallPort.RemoveSmallPortDriver;
var
  hService: SC_HANDLE;
begin
  dwLastError := 0;
...
procedure TKMSmallPort.StarSSSmallPortService;
var
  hService: SC_HANDLE;
  lpVectors: PChar;
begin
...
procedure TKMSmallPort.StopSmallPortService;
var
  hService: SC_HANDLE;
